# Hymoex Skills Distribution

**Official repository for distributing Hymoex-related skills for AI coding agents.**

This repository provides installable skills for:
- Claude Code
- Cursor  
- Windsurf
- Other AI coding agents

---

## 📦 Available Skills

### [Hymoex Architect](hymoex-architect/) ⭐

Interactive architectural guidance for building multi-agent systems using Hymoex patterns.

**Quick Install:**
```bash
curl -fsSL https://raw.githubusercontent.com/Pymut/hymoex/main/apps/skills/installers/install-architect.sh | bash
```

---

## 🎯 About Hymoex

Hymoex is a cognitive architecture for building multi-agent systems. Learn more at the [main Hymoex repository](https://github.com/Pymut/hymoex).

---

## 🤝 Contributing

Want to add a Hymoex-related skill? See [CONTRIBUTING.md](CONTRIBUTING.md).

---

**Made with ❤️ by the Pymut lab**
